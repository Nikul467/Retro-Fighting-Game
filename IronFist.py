...
class Player(Entity):
    ...
    def double_jump(self):
        ...
        if self.clocks.timer == 1:
            self.create_hitbox(self, 10, -35, 30, 6,8)
...
class Interaction:
    ...
    def add_enemy(self):
        ...
        enemy = Ghost(self.game, Vector((0 if direction == "right" else self.game.WIDTH),(random.uniform(0,self.game.FLOOR-40))),20)
    ...
    def update(self):
        ...
        if self.stop_timer > 0:
            ...
            if (self.keyboard.up and self.player.jumps ==1 and self.player.state is "airborne") or (self.keyboard.up and self.player.hit_cancel):
                self.player.jumps = 2
                self.player.double_jump_init()
                self.keyboard.up = False
                self.player.direction_force(self.player.direction)
                
            elif self.keyboard.up and self.player.on_ground() and self.player.state is not "attack" and self.player.state is not "air_attack":
                self.player.jump()
                self.keyboard.up = False
                self.player.jumps = 1
                self.player.direction_force(self.player.direction)
            ...
            for enemy in self.enemies:
                ...
                if self.player.hitbox is not None and self.player.hitbox.hit(enemy.hurtbox):
                    self.player.hit()
                    self.stop_timer = self.player.hitbox.hitlag
                    enemy.die()  # Kill the enemy if it collides with the player
                    removed.add(enemy)
